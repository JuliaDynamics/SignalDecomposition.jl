...
"""
    TimeAnomaly()
Decompose timeseries `s` into a **sum** `x + r` where `x` is the same-time average and
`r` is the anomalies. Each unique day+month combination in `t` is identified, and the
